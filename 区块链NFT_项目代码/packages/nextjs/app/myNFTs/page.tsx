...
import { saveNFTToDB } from '~~/services/mysql/dbConfig';
...
const MyNFTs: NextPage = () => {
  ...
  // console.log('tokenIdCounter',Number(tokenIdCounter))
  ...
  const handleMintItem = async () => {
    const { image, id, name, attributes, owner, price, description, timestamp } = nftInfo;
    if (image === "") {
    ...
    try {
      const uploadedItem = await addToIPFS({ image, id, name, attributes, owner, price, description, timestamp });
      ...
      if (tokenIdCounter !== undefined) {
        ...
        const nftdata = {
          NFT_name: newNftInfo.name,
          Description: newNftInfo.description,
          Image_URL: newNftInfo.image,
          Owner_Address: newNftInfo.owner,
          Token_ID: (newNftInfo.id).toString()


        }
        const data = await saveNFTToDB(nftdata);
